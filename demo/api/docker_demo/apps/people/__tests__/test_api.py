...
from docker_demo.apps.people import models
...
def test_me_unauthenticated(client: Client):
    ...
    response = client.get(
        reverse(f"api-{metadata.version('docker_demo')}:me")
    )
...
def test_login(client: Client, admin_user: models.User):
    ...
    response = client.post(
        reverse(f"api-{metadata.version('docker_demo')}:login_user"),
...
def test_logout(admin_client: Client):
    ...
    response = admin_client.post(
        reverse(f"api-{metadata.version('docker_demo')}:logout_user"),
    )
    data = response.json()
